...
> **【実装決定 2026-07-19】乾いた砂の「攪乱」表現への転換**
>
> 参考写真(乾いた珊瑚砂のビーチ)との比較検証の結果、既定の足跡表現を
> 「解剖学的な肉球+指球のスタンプ」から **「砂の攪乱」スタンプ** に変更した。
> 現実の乾いた砂では肉球の形はくっきり残らず、跡は
> 「崩れた不定形の窪み + 進行方向の逆側へ押し出された縁 + 蹴り散らしの砂だまり」
> として残るため(dog-3d-app で先行実装した表現の移植)。
>
> - `pawStamp.ts` の `paintDisturbance` が R=窪みマスク / G=盛り上がりマスク を生成。
>   輪郭は低次ローブ+ノイズで歪ませ、蹴り散らしは後方(local -y)へ配置。
> - 旧表現は `dogPawPrint`(`footprintTypes.ts`)として保持(湿った砂など将来の切替用)。
> - 乾いた砂は色がほぼ変わらないため、暗化(darkening)は控えめにし、
>   仰角約35°の斜光(§9)+法線再構成による**陰影**で凹凸を読ませる。
> - **注意**: compose シェーダーでハイトフィールドにテクセル単位のホワイトノイズを
>   乗せてはならない。隣接差分による法線が乱れ、点描状のアーティファクトになる
>   (崩れ表現はスタンプ側に低周波で焼き込む)。
> - `MAX_FOOTPRINTS` は 15→**40** に拡張。15個ではひと続きのなぞりで古い跡が
>   すぐバッファ切れで消え、「砂に跡が残る」体験が成立しないため
>   (減衰寿命 ~10秒とのバランスで決定)。
> - この決定は z.ai GLM-5 と Claude Fable の協働レビュー
>   (ユーザー目的からの設計捉え直し×2サイクル)を経て確定。
>   サイクル1で蹴り散らしの方向バグ(前方に飛んでいた)を検出・修正。
